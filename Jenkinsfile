...
def xcodeproj = 'JenkinsExample.xcodeproj' // Path to the xcodeproj
def xcarchive_name = "JenkinsExample.xcarchive" // Name of the archive to build
def build_scheme = 'JenkinsExample' // Scheme to build the app
def test_scheme = 'JenkinsExample' // Scheme to build tests
def simulator_device = "platform=iOS Simulator,name=iPhone 6,OS=11.2" // Name of the device type to use for tests
...
sh 'xcodebuild clean -project JenkinsExample.xcodeproj -configuration Release -alltargets'
sh 'xcodebuild archive -project JenkinsExample.xcodeproj -scheme Testing -archivePath JenkinsExample.xcarchive -allowProvisioningUpdates'
sh 'xcodebuild -exportArchive -archivePath JenkinsExample.xcarchive -exportPath JenkinsExample.ipa -exportOptionsPlist exportOptions.plist'




                //sh "xcrun xcodebuild -scheme '${build_scheme}' -destination platform='iOS Simulator',name='iPhone 6',OS=11.2 clean build | tee build/xcodebuild.log | xcpretty"
                
            //sh 'xcodebuild clean -project JenkinsExample.xcodeproj -configuration Release -destination platform="iOS",name="iPhone 6",OS=11.2 -allowProvisioningUpdates' 
          ////  sh 'xcodebuild clean -project JenkinsExample.xcodeproj -configuration Release -alltargets' 
        /////sh 'xcodebuild archive -project JenkinsExample.xcodeproj -scheme Testing -archivePath JenkinsExample.xcarchive -allowProvisioningUpdates'
        ...
        //sh 'xcodebuild -scheme "Testing" -sdk "iphoneos" -configuration Debug build'
        

        //# Package the application
        //sh '/usr/bin/xcrun -sdk "${TARGET_SDK}" PackageApplication -v "${PROJECT_BUILDDIR}/${SCHEME_NAME}.app" -o "${BUILD_OUTPUT_DIR}/${APP_NAME}.ipa" --sign "${DEVELOPER_NAME}" --embed "${PROVISIONING_PROFILE}"'
        
        
        
            
//            sh 'xcodebuild -exportArchive -archivePath JenkinsExample.xcarchive -exportPath JenkinsExample -exportFormat ipa -exportProvisioningProfile "AppStore Distribution Profile"'
            
                
//     sh 'xcodebuild -scheme "Testing" -configuration "Debug" build test -destination "platform=iOS Simulator,name=iPhone 6,OS=11.2" -destination "platform=iOS Simulator,name=iPhone 7,OS=11.2" clean build | tee buildABCD/xcodebuild.log | xcpretty'
//                 // Uncomment this when building a project with code signing set up
//                 sh "xcrun xcodebuild -scheme 'Testing' archive -archivePath 'build/${xcarchive_name}' | tee buildABCD/xcodebuild.log | xcpretty"
//                 sh "xcrun xcodebuild -exportArchive -exportOptionsPlist exportOptions.plist -archivePath 'build/${xcarchive_name}' -exportPath buildABCD"
//                 dir('buildABCD') {
//                     echo 'Zip & Move file....'
//                     sh "zip -qr 'JenkinsExample-${env.BUILD_NUMBER}.zip' '${xcarchive_name}'"
//                     sh "mv 'JenkinsExample.ipa' 'JenkinsExample-${branchNameForURL}.ipa'"
//  //               }
//             }
        }
...
//                     sh "xcodebuild -scheme '${test_scheme}' -enableCodeCoverage YES -configuration Debug -destination platform=iOS Simulator,name=iPhone 6,OS=11.2 build-for-testing | tee build/xcodebuild-test.log | xcpretty"

//                     // Determine the location of the apps
//                     app_path = sh(script: "xcodebuild -scheme '${test_scheme}' -configuration Debug -sdk iphonesimulator -destination platform=iOS Simulator,name=iPhone 6,OS=11.2 build -showBuildSettings CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO 2>/dev/null | grep CODESIGNING_FOLDER_PATH | awk -F' = ' '{ gsub(\"Build/Products\", \"Build/Intermediates/CodeCoverage/Products\", \$2); print \$2 }'", returnStdout: true).trim()
...
//                     parallel (
//                         "Unit Tests": {
//                             sh "bluepill -a '${app_path}' -s '${xcodeproj}/xcshareddata/xcschemes/JenkinsExampleTests.xcscheme' -o build/reports-tests -S 180 -T 450 -n 1 || true"
//                         },
//                         "UI Tests": {
//                             sh "bluepill -a '${app_path}' -s '${xcodeproj}/xcshareddata/xcschemes/JenkinsExampleUITests.xcscheme' -o build/reports-uitests -S 180 -T 450 -n 1 || true"
//                         }
//                     )
...
//                     sh "xcodebuild -scheme '${test_scheme}' -enableCodeCoverage YES -configuration Debug -destination platform=iOS Simulator,name=iPhone 6,OS=11.2 test | tee build/xcodebuild-test.log | xcpretty -r junit --output build/reports/junit.xml"
//                     sh "slather coverage --scheme '${test_scheme}' --cobertura-xml --output-directory build/coverage '${xcodeproj}'"
...
//                     //sh "xcodebuild -scheme 'JenkinsExample iPad' -enableCodeCoverage YES -configuration Debug -destination 'name=iPad Air 2' test | tee build/xcodebuild-ipad-test.log | xcpretty -r junit --output build/reports/junit-ipad.xml"
//                     //sh "slather coverage --scheme 'JenkinsExample iPad' --cobertura-xml --output-directory build/coverage '${xcodeproj}'"
